...
if [ -z "$TRITON_WHEEL" ]; then
  ...
  # Re-glob via the same ABI-aware helper used for the cache-hit check above
  # -- a plain hash-only glob here could pick an older cached wheel for the
  # same hash but a different CPython ABI instead of the one just built.
  TRITON_WHEEL=$(has_triton_wheel "$TRITON_WHEEL_OUTPUT_DIR" "$TRITON_GIT8")
