...
class FlashOpSourceMonad(Monad):
    ...
    def preprocess(self, tup):
        ...
        if skip_bwd:
            ...
            if (causal or bias_type != 0) and seqlen_q * seqlen_k * D_HEAD >= 2048 * 2048 * VRAM_CAP_IN_GB:
                ...
                N_HEADS = min(N_HEADS, 2)
                BATCH = min(BATCH, 2)
        return (BATCH, N_HEADS, D_HEAD, seqlen_q, seqlen_k, causal, sm_scale, dropout_p, return_encoded_softmax, dtype, bias_type, op_backend)

    def gen_from_argv(self):
        a = self._args
        for tup in itertools.product(a.batch, a.n_heads, a.d_head, a.seqlen_q, a.seqlen_k, a.causal, a.sm_scale, a.dropout_p, a.return_encoded_softmax, a.dtype, a.bias_type, a.op_backends):
...
def parse():
    ...
    p.add_argument('--op_backends', type=int, nargs=NARG_PLUS, default=[0, 1, 2], choices=[0, 1, 2], help='Operator Backends to profile, 0: Split, 1: Fuse, 2: AITER ASM.')
...
def main():
    ...
    args = parse()
    tuner = FlashOpTunerManager(args)
    tuner.build_graph()
    app = make_ui(tuner)
    tuner.launch_graph()
