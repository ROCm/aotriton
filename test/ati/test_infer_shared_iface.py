...
def test_metro_subkernels_get_operator():
    k1 = _Kernel('attn_fwd')
    debug = _Kernel('debug')
    alt = _Kernel('aiter')                       # alternative backend
    op = _Op('op_attn_fwd', [_Metro([k1, _Cond(debug)]), alt])
    ...
    assert alt.SHARED_IFACE is None              # alternative backend untouched
...
def test_default_backend_only():
    # backends[1+] are alternatives — interchangeable, not the struct source.
    main = _Kernel('main')
    alt = _Kernel('alt')
    op = _Op('op', [_Metro([main]), _Metro([alt])])
    infer_shared_iface([op])
    assert main.SHARED_IFACE is op
    assert alt.SHARED_IFACE is None
