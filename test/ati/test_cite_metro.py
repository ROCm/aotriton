...
def _register_fwd_op():
    ...
    af = _build_attn_fwd_kdesc()                  # also flat-registers attn_fwd
...
def test_subkernel_cite_via_ops():
    _register_fwd_op()
    kdesc = _cite_debug('op_attn_fwd.triton.attn_fwd')
...
def test_whole_metro_cite_via_ops():
    _register_fwd_op()
    kdesc = _cite_debug('op_attn_fwd.triton')    # whole metro, no sub-kernel
    assert kdesc.axis_of_arg('dropout_p') is not None
    R = next(a for a in kdesc._built.axes if 'R' in a.arg_names)
    assert R.radix == 3
