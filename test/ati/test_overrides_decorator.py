...
def test_literal_zero_override():
    ...
    c = ov.materialize({'RETURN_ENCODED_SOFTMAX': Choice.parse(False)})
...
def test_string_to_is_varref():
    ...
    picked = {'PADDED_HEAD': Choice.parse(False), 'BLOCK_DMODEL': Choice.parse(64)}
...
def test_ne_predicate_sliding_window():
    ...
    assert ov.predicate.holds({'CAUSAL_TYPE': Choice.parse(0)})
    assert not ov.predicate.holds({'CAUSAL_TYPE': Choice.parse(3)})
