...
"""Aux-kernel xref Step 12 (part 1): bwd_kernel_dk_dv ported to ATI
...
def test_struct_has_bwd_fields():
    ...
    assert fields['DK'] == 'const TensorView<4>*'
    assert fields['DV'] == 'const TensorView<4>*'
...
def main():
    ...
    for fn in fns:
        fn()
    print(f'OK: {len(fns)} port-bwd-dkdv tests passed.')
    return 0
