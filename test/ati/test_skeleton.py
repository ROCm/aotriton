...
def _check_stubs_raise():
    # Calling an unimplemented entry point must fail loudly, not silently no-op.
    callables = [
        lambda: ati.tensor_dtype('T'),
        lambda: ati.tensor('Q', 'T'),
        lambda: ati.scalar('x', 'i32'),
        lambda: ati.overrides('B', to=0),
