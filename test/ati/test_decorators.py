...
def test_tensor_dtype_and_binding_share_var():
    T = ati.tensor_dtype('T_io', dtype=['*fp16:16', '*bf16:16', '*fp32:16'])
    assert isinstance(T, ChoiceVar) and T.kind == 'dtype'
...
def test_stride_glob_match_in_signature_order():
    spec = ati.tensor('Q', ati.tensor_dtype('T', dtype=['*fp16:16']),
                      strides='stride_q?')
...
def test_scalar_shared_choice_var():
    S = ati.choice_set('Seqlen', options=['i32'])
