...
"""Unit test for tune.configs/binning/fallback/derived registration (Step 3.2)."""
...
def _describe(k):
    ...
    describe(k,
             ...
             ati.tune.derived(NUM_XCDS=lambda f: 8),
             _validate=False)
...
def test_tune_spec_assembled():
    ...
    assert ts.schema is not None and ts.schema.names == ['BLOCK_M', 'PRE_LOAD_V']
    assert set(ts.binning) == {'Max_seqlen_q', 'Max_seqlen_k'}
    assert ts.fallback == {'PADDED_HEAD': False}
    assert set(ts.derived) == {'NUM_XCDS'}
...
def test_configs_generator_runs_over_functional():
    ...
    axes = [Axis('CAUSAL_TYPE', ('CAUSAL_TYPE',),
                 [Choice.parse(0), Choice.parse(3)], anchor=0)]
