...
class ProbeHandler(MessageHandler):
    ...
    def _build_fanout(self, impl_dict: dict, task_id: int,
        ...
        # only its position (impl_index) matters. That is why the op level
        # carries a NAME rather than an index: position and backend index
        # diverge wherever the runnable backends are not a contiguous prefix
        # of the operator's list (gfx1201: triton=0, flyc=2). Storage stays bare
