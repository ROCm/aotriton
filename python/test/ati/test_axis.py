...
from aotriton.template_instantiation.ir import TypedChoice, Axis, assign_godel, godel_of


def _axis(var_name, raw_choices, anchor):
    return Axis(var_name,
                arg_names=(var_name,),
                choices=[TypedChoice.parse(c) for c in raw_choices],
                anchor=anchor)
...
def test_choice_for_arg_rank_specialization():
    axis = Axis('T_io', arg_names=('Q', 'L'),
                choices=[TypedChoice.parse('*fp16:16')],
