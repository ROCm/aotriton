...
def bwd_inner_dk_dv_fuse(
    ...
    for start_q in range(lo, hi, BLOCK_M):
        ...
        if not FULL_BLOCKS:
            ...
            o = load_fn(o_ptrs, offs_q + start_q, ld_offs_d, seqlen_q, head_dim)
        else:
            do = load_fn(do_ptrs, None, ld_offs_d, seqlen_q, head_dim)
            o = load_fn(o_ptrs, None, ld_offs_d, seqlen_q, head_dim)
        # -- compute qk ----
        ...
        Di = tl.sum(o.to(tl.float32) * do.to(tl.float32), axis=1)  # (BLOCK_M,)
        if FULL_BLOCKS:
            l_i = tl.load(l_ptrs + offs_q_curr)
        else:
            boundary = tl.full((BLOCK_M, ), BLOCK_M - overflow_size, dtype=tl.int32)
