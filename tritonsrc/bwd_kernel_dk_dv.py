...
def bwd_kernel_dk_dv(
    ...
    if PERSISTENT and not unsupported_by_persistent:  # Only enable for non-varlen
        ...
        num_tiles_per_head = tl.cdiv(max_seqlen_k, BLOCK_N)  # the number of work units (tiles) of a single head
        num_tiles_per_sample = num_tiles_per_head * num_head_k  # times the number of heads
    ...
    while tile_id < num_tiles_total:  # loops more than once only if PERSISTENT
        if PERSISTENT_DYNAMIC and not unsupported_by_persistent:
            # tile id basically tells us the K block we are handling
            off_z = tile_id // num_tiles_per_sample  # at which batch sample are we
            off_h_k = tile_id % num_tiles_per_sample // num_tiles_per_head  # at which head are we inside the sample
            start_k = tile_id % num_tiles_per_sample % num_tiles_per_head * BLOCK_N  # at which tile are we inside the head
        else:
        ...
        if continue_condition:
            ...
            if IS_CAUSAL:
                if IS_CAUSAL_BOTTOM_RIGHT:
                    # Bottom right causal variant - adjust q_lo based on start_k
                    causal_offset = seqlen_q - seqlen_k if seqlen_q > seqlen_k else 0
                    q_lo = max(start_k - causal_offset, 0) if start_k >= causal_offset else 0
                else:
                    # Top left causal variant
                    q_lo = start_k
            q_hi = seqlen_q
            ...
            if IS_CAUSAL:
                ...
                if IS_CAUSAL_BOTTOM_RIGHT:
                    # For bottom-right causal, leading blocks calculation is different
                    if start_k >= causal_offset:
                        diag_start = start_k - causal_offset
                        leading_masked_blocks = tl.cdiv(diag_start, BLOCK_M)
                    else:
                        leading_masked_blocks = 0
                else:
            ...
            if n_full_blocks < 0:
                n_full_blocks = 0  # Avoid negative block count
        ...
        # Update tile_id for next iteration or exit loop
