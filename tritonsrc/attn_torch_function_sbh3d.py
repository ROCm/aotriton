...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, b, causal, sm_scale, dropout_p, return_encoded_softmax,
        ...
        batch = q.shape[0]
        num_head = q.shape[1]
        o = torch.zeros((max_seqlens_q, batch, num_head, Lq), device=q.device, dtype=q.dtype)
        o = torch.permute(o, (1, 2, 0, 3))
        print("o.shape: ", o.shape, ", o.stride(): ", o.stride())
    ...
    def backward(ctx, do, _, fwd_tuning_result):
        ...
        qkvdims = (max_seqlens_q, batch, num_head, 3, Lq)

        dqkv = torch.randn(qkvdims, dtype=q.dtype, device="cuda", requires_grad=True)
        dqkv = torch.permute(dqkv, (1, 2, 0, 3, 4))
        dq = dqkv[:, :, :, 0, :]
        dk = dqkv[:, :, :, 1, :]
        dv = dqkv[:, :, :, 2, :]
