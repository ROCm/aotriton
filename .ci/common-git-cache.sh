...
if [[ -n "${pat_environ}" ]]; then
  ...
  #
  # The credential.helper VALUE itself becomes part of the `git` process's
  # own argv (visible via `ps`/`docker top` while the fetch runs, `set -x`
  # or not) -- so it must never contain the raw token. Export the token
  # under a fixed name and have the helper reference that name, not the
  # token's value: only the (harmless) variable name crosses into argv, the
  # secret itself stays in the environment.
  set +x
  export AOTRITON_GIT_PAT="${!pat_environ}"
  git -C /mirror -c credential.helper='!f() { echo username=x-access-token; echo "password=$AOTRITON_GIT_PAT"; }; f' \
    fetch --prune origin
