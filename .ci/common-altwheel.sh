...
# entry's origin (see .ci/AltWheelExample.yaml). Empty if unset -- no PAT is
...
# Wheel filenames pip produces embed the CPython ABI tag as cp<major><minor>
# (no dot), e.g. cp311 for Python 3.11 -- derive the glob fragment from a
# plain "X.Y" version string.
altwheel_abi_glob() {
  local pyver="$1"
  echo "cp${pyver//./}"
}

# Find the built wheel matching <hash> AND <pyver>'s ABI in <host_wheel_dir>
# (a directory of triton-*+git<hash8>*-cp<XY>-*.whl files, e.g. .ci's
# WHEEL_CACHE_DIR or .tune's <workdir>/scratch/triton). The cache can hold
# wheels for more than one Python version at once (build_triton_wheels.sh
# accepts --python) -- matching on hash alone can silently resolve a
# different venv's ABI-incompatible wheel. Prints the absolute path, or
# nothing if missing.
altwheel_resolve_wheel_path() {
  local host_wheel_dir="$1" hash="$2" pyver="$3" short abi
  short="${hash:0:8}"
  abi="$(altwheel_abi_glob "${pyver}")"
  ls "${host_wheel_dir}"/triton-*+git${short}*"${abi}"*.whl 2>/dev/null | head -n1
}
...
#                       yaml, which has no default key).
# <pyver>               "X.Y" Python version the resolved wheels must match
#                       (see altwheel_resolve_wheel_path) -- the same value
#                       passed as build_triton_wheels.sh's --python for this
#                       host_wheel_dir.
altwheel_resolve_config() {
  local yaml="$1" host_wheel_dir="$2" container_wheel_dir="$3" default_hash="$4" pyver="$5"
  ...
  host_wheel=$(altwheel_resolve_wheel_path "${host_wheel_dir}" "${resolved_default_hash}" "${pyver}")
  if [ -z "${host_wheel}" ]; then
    echo "Error: no built wheel found for altwheel default venv (hash ${resolved_default_hash:0:8}, python ${pyver})" >&2
    return 1
  fi
  # Build one combined yq filter expression instead of a separate read-
  # modify-write `yq -i` pass per key -- N venv entries previously meant N
  # full-file rewrites; this applies all of them in a single pass.
  local expr=".venvs.default = \"${container_wheel_dir}/$(basename "${host_wheel}")\""

  for key in $(yq -r '.venvs | keys | .[]' "${yaml}"); do
    [ "${key}" = "default" ] && continue
    hash=$(altwheel_venv_hash "${yaml}" ".venvs.${key}")
    host_wheel=$(altwheel_resolve_wheel_path "${host_wheel_dir}" "${hash}" "${pyver}")
    if [ -z "${host_wheel}" ]; then
      echo "Error: no built wheel found for altwheel venv '${key}' (hash ${hash:0:8}, python ${pyver})" >&2
      return 1
    fi
    expr="${expr} | .venvs.${key} = \"${container_wheel_dir}/$(basename "${host_wheel}")\""
  done
